...
regimes <- function(lon, lat, field, ncluster = 4, ntime = 1000, neof = 10, xlim, ylim, alg = "Hartigan-Wong", flag_perc = FALSE, perc = NULL, max_eofs = 50) {
  ...
  if (flag_perc) {
    if (is.null(perc)) {
      print('flag_perc is TRUE but no perc as been specified')
    }
    reducedspace <- eofs(lon, lat, field, neof = max_eofs, xlim = xlim, ylim = ylim, method = "SVD", do_regression = F, do_standardize = F)
    #print(dim(reducedspace$coeff))
    #print(cumsum(reducedspace$variance))
  ...
  # prepare output
  print("Finalize...")
  out <- list(cluster = cluster, frequencies = frequencies[kk], regimes = compose[, , kk], pcs = PC, clus_centers = regimes$centers[kk,], tot.withinss = regimes$tot.withinss)
  return(out)
}
